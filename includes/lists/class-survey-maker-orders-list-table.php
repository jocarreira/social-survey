<?php
class Survey_Orders_List_Table extends WP_List_Table {
    private $plugin_name;
    /** Class constructor */
    ...
    public static function get_orders( $per_page = 20, $page_number = 1 ) {
        ...
        $sql = "SELECT * FROM {$wpdb->prefix}socialsurv_orders";

        if( ! $current_user_can_edit ){
            $sql = "SELECT o.* 
                FROM {$wpdb->prefix}socialsurv_orders AS o
                LEFT JOIN {$wpdb->prefix}socialsurv_surveys AS s
                    ON o.survey_id = s.id
                WHERE s.author_id = {$current_user} 
                ";
        }
    ...
    public static function record_count() {
        ...
        $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}socialsurv_orders";
    ...
    public function no_items() {
        Survey_Maker_Data::survey_no_items_list_tables('orders');
    }
    ...
    function column_survey_id( $item ) {
        ...
        $result = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}socialsurv_surveys WHERE id={$item['survey_id']}", "ARRAY_A");

        $title = Survey_Maker_Admin::ays_restriction_string("word",stripcslashes($result['title']), 5);
    ...
    function get_columns() {
        $columns = array(
            ...
            'survey_id'             => __( 'Survey', $this->plugin_name ),
    ...
    public function get_sortable_columns() {
        $sortable_columns = array(
            'survey_id'         => array( 'survey_id', true ),
    ...
    public function prepare_items() {
            ...
            $per_page = $this->get_items_per_page('survey_orders_per_page', 20);
