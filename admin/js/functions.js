// (function($){
    ...
    function openMediaUploaderForGifLoader(e, element) {
        ...
        var aysUploader = wp.media({
            ...
            wrap.find('a.add_survey_loader_custom_gif').hide();
        }).open();
        return false;
    }
...
//    }
    ...
    function selectElementContents(el) {
        if (window.getSelection && document.createRange) {
            ...
            if(jQuery(el).hasClass("ays-survey-question-id-copy-box")){
                text      = jQuery(el).find(".ays-survey-question-id-copy").html();
            }
    ...
    function aysSurveyRgba2hex(orig, isAlpha) {
        if( orig.indexOf('#') !== -1 ){
            if( isAlpha ){
                orig += "29";
            }
    ...
    function openMediaUploaderForConditions(e, element) {
        ...
        var aysUploader = wp.media({
            ...
            var attachment = aysUploader.state().get('selection').first().toJSON();
            var $this = element;
            var $thisParent = $this.parents('.ays-survey-email-message-files');
            $thisParent.find('input.ays-survey-email-message-editor-file').val(attachment.url);
            $thisParent.find('.ays-survey-email-message-files-content-text').html(attachment.filename);
            $thisParent.find('.ays-survey-email-message-files-content-text').attr("href" , attachment.url);
            $thisParent.find('.ays-survey-email-message-files-content-text').attr("download" , attachment.url);
            $thisParent.find('.ays-survey-email-message-editor-file-name').val(attachment.filename);
            $thisParent.find('.ays-survey-email-message-files-wrapper').removeClass("display_none_not_important");
            $thisParent.find('.ays-survey-email-message-files-body').removeClass("display_none_not_important");
            $thisParent.find('.ays-survey-add-email-message-files').html("Edit file");
            $thisParent.find('.ays-survey-email-message-editor-file-id').val(attachment.id);
        }).open();
        return false;
    }
    ...
    function getEachSubmission(e){
        ...
        // if(!notToTrigger.hasClass("ays-survey-no-trigger")){
